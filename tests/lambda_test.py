...
import json
import logging
import src.lambda_function as l
import unittest
import utils
import yaml
...
class TestLambdaFunction(unittest.TestCase):
    ...
    def test_latest_csv_file(self):
        ...
        keys = l._latest_csv_keys(s3client, env_var, event_time)
    ...
    def test_missing_environment_variable(self):
        ...
        for v in env_vars:
            tmp = os.environ[v]
            del os.environ[v]
            self.assertRaises(KeyError, l.lambda_handler, None, None)
            os.environ[v] = tmp
    ...
    def test_parsed_logs(self):
        ...
        with gzip.open(SAMPLE_CSV_GZIP_1) as f:
            ...
            event_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            l._parse_file(csv_lines, self._logzio_url, event_time)
    ...
    def test_multi_reports(self):
        ...
        latest_csv_keys = l._latest_csv_keys(s3client, env_var, event_time)
        readers = []

        # first csv
        csv_like_obj1 = s3client.get_object(Bucket=env_var['bucket'], Key=latest_csv_keys[0])
        csv_obj1 = l._download_object(csv_like_obj1, True)
        ...
        csv_obj2 = l._download_object(csv_like_obj2, True)
        ...
        httpretty.register_uri(httpretty.POST, self._logzio_url)
        httpretty.enable()
        l._parse_file(csv_lines1, self._logzio_url, event_time)
        l._parse_file(csv_lines2, self._logzio_url, event_time)
